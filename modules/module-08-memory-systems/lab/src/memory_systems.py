...
EMBED_URL = (
    f"{_EMBED_ENDPOINT}/openai/deployments/{EMBEDDING_DEPLOYMENT}"
    f"/embeddings?api-version={_EMBED_API_VERSION}"
)
EMBED_HEADERS = {"api-key": _EMBED_API_KEY, "Content-Type": "application/json"}
...
async def embed_text(text: str) -> list[float]:
    ...
    Steps:
    1. POST to EMBED_URL with headers=EMBED_HEADERS and body: {"input": text}
